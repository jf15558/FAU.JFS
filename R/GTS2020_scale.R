...
GTS2020_scale <- function(x, srt = "early_interval", end = "late_interval", max_ma = NULL, min_ma = NULL) {
  ...
  if(length(test) > 0) {
    warning(paste0("The following intervals are not present in the GTS2020 lookup:",
                   paste0(test, collapse = ", "), ". If max_ma and min_ma have been supplied,
                   these values will be supplied for the unmatched intervals. Otherwise, NA
