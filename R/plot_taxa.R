...
plot_taxa <- function(x, taxon, trank, ranks, mode = c("parent", "child", "all"), step = NULL) {
  ...
  # get the node in the tgraph pertaining to that taxon
  tnode <- which(igraph::V(tg$taxa)$name %in% taxon)
  # get directed paths for the taxon
  ...
  rseq <- seq(from = -1, to = 1, length.out = length(unique(igraph::V(try)$rank)))
  par(mar = c(0, 5, 0, 5))
  par(las = 1)
  plot(try, layout = lay$layout, edge.arrow.mode = 0, margin = 0.2)
  axis(4, labels = rev(names(tg$ranks)[unique(igraph::V(try)$rank[order(igraph::V(try)$rank)])]), at = rseq, col = NA)
}
